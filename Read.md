...
3. 바탕화면에 `DesktopPet.app` 빌드 (강아지 사진을 아이콘으로)
...
./make_app.sh    # .app 아이콘도 새 사진으로 갱신
```

`rembg` 가 배경을 자동 제거합니다. 옆모습이면 걷기 애니메이션이 자연스럽지만 정면 사진도 OK.
...
업데이트 받으려면 `git pull && ./setup.sh` (의존성 변동 시).

## 파일 구조

```
pet.py             메인 앱 (PyQt6 + 상태 머신 + 말풍선)
monitor.py         하드웨어 샘플러 (psutil + macmon 래퍼 + 코어 레이아웃 감지)
prep_sprite.py     원본 사진 → 투명 배경 PNG (rembg)
make_app.sh        바탕화면에 DesktopPet.app 빌드
setup.sh           초기 셋업 (한 번만 실행)
run.sh             터미널에서 직접 실행
requirements.txt   Python 의존성 목록
assets/dog.png     펫 스프라이트 (배경 제거된 PNG)
```
